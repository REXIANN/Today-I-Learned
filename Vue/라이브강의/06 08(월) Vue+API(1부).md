...
- import를 할 때 뒤에있는 확장자명은 생략이 가능하다
...
- index 라는 이름은 특별하다. 폴더 안의 파일이 지정되어 있지 않으면 자동으로 index파일을 불러온다
...
// store 폴더에 있는 index.js가 실질적으로 import 된다
```
...
☝️이벤트리스너 함수(ex: @keypress.enter)는 실행할 때 자동으로 event 가 넘어온다. 자동으로 실행되는 시점에 event라는 것을 넘겨준다. 검색창의 경우 `event.target.value` 에 검색어가 자동으로 들어 있다.